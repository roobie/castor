...
# Full CI workflow (build castor + install + test)
just ci
...
- `castor_binary` - Path to compiled Castor binary
- `castor_store` - Empty store directory (not initialized)
...
```python
def test_workflow(cli, castor_store, workspace):
    ...
    hash_val = cli.add(data, root=castor_store, ref_name="backup").stdout.strip()

    # Verify
    verify_object_exists(castor_store, hash_val)

    # Restore
    restore_dir = workspace / "restored"
    cli.materialize(hash_val, restore_dir, root=castor_store)
...
def test_init(cli, castor_store):
    ...
...
```
FileNotFoundError: Castor binary not found at /workspace/target/debug/castor
```
...
jobs:
  test:
    ...
    steps:
      ...
      - name: Build Castor
        run: cargo build --release -p castor
