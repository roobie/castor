...
use castor_core::{Algorithm, Hash, Store};
...
#[command(version)]
struct Cli {
    /// Store root directory (defaults to CASTOR_ROOT env var or ./castor-store)
...
fn main() -> Result<()> {
    ...
    // Determine store root: CLI arg > CASTOR_ROOT env var > ./castor-store default
    let root = cli
        .root
        .or_else(|| std::env::var("CASTOR_ROOT").ok().map(PathBuf::from))
        .unwrap_or_else(|| PathBuf::from("./castor-store"));
...
fn cmd_init(root: &Path, algo: &str) -> Result<()> {
    ...
    println!("Initialized castor store at {}", root.display());
            ...
            for entry in entries {
                if long {
                    let type_char = match entry.entry_type {
                        castor_core::EntryType::Blob => 'b',
                        castor_core::EntryType::Tree => 't',
                    };
                    println!(
                        "{} {:06o} {} {}",
