...
`castor_core` is a Rust library that provides the core functionality for **Castor**, a minimal, single-binary content-addressed storage system. It stores files and directories by their cryptographic hash, ensuring immutable, deduplicated storage with built-in garbage collection.
...
```rust
use castor_core::{Store, Algorithm};
use std::path::Path;
...
```bash
# Build the library
cargo build --release -p castor_core

# Run all tests (68 unit tests + 1 doctest)
cargo test -p castor_core

# Run with output
cargo test -p castor_core -- --nocapture

# Check code quality
cargo clippy -p castor_core -- -D warnings

# Format code
cargo fmt -p castor_core
```
...
1. Ensure all tests pass: `cargo test -p castor_core`
2. Maintain clippy cleanliness: `cargo clippy -p castor_core -- -D warnings`
3. Format code: `cargo fmt -p castor_core`
...
- **castor** - CLI binary using this library (coming soon)
