...
@pytest.mark.smoke
def test_full_backup_restore_workflow(cli, casq_store, workspace):
    ...
    cli.materialize(tree_hash, restore_dir, root=casq_store)
...
def test_complex_multicommand_scenario(cli, casq_store, workspace):
    ...
    cli.materialize(hash_v1, restore_v1, root=casq_store)
...
def test_store_migration_workflow(cli, casq_store, workspace):
    ...
    for i in range(10):
        ...
        if i % 2 == 0:
            ...
            cli.add(file, root=casq_store)

    initial_count = count_objects(casq_store)
    assert initial_count > 0

    # 3. GC
    gc_result = cli.gc(root=casq_store)
    assert gc_result.returncode == 0

    final_count = count_objects(casq_store)
