...
def test_ls_refs_multiple_refs(cli, initialized_store, workspace):
    ...
    assert result.returncode == 0
    assert "ref1" in result.stderr
    assert "ref2" in result.stderr
    assert "ref3" in result.stderr
...
def test_ls_refs_long_format(cli, initialized_store, sample_file):
    ...
    assert "myref" in result.stderr
    assert file_hash in result.stderr
...
def test_ls_blob_short_format(cli, initialized_store, sample_file):
    ...
    assert "blob" in result.stderr.lower() or file_hash in result.stderr
...
def test_ls_blob_long_format(cli, initialized_store, sample_file):
    ...
    assert "blob" in result.stderr.lower() or len(result.stderr) > 0
...
def test_ls_tree_short_format(cli, initialized_store, sample_tree):
    ...
    assert "file2.txt" in result.stderr
...
def test_ls_empty_tree(cli, initialized_store, workspace):
    ...
    assert result.stderr.strip() == "" or "0" in result.stderr
...
def test_ls_nested_tree(cli, initialized_store, nested_tree):
    ...
    assert "top.txt" in result.stderr
    assert "dir1" in result.stderr or "tree" in result.stderr
    assert "dir2" in result.stderr or "tree" in result.stderr
...
def test_ls_tree_entries_sorted(cli, initialized_store, workspace):
    ...
    assert "middle.txt" in result.stderr
    assert "zebra.txt" in result.stderr
...
def test_ls_shows_mode_in_long_format(cli, initialized_store, workspace):
    ...
    assert "script.sh" in result.stderr
...
def test_ls_blob_shows_size(cli, initialized_store, workspace):
    ...
    assert "1000" in result.stderr or "blob" in result.stderr.lower()
...
def test_ls_tree_with_dotfiles(cli, initialized_store, workspace):
    ...
    assert "visible.txt" in result.stderr
...
def test_ls_large_tree(cli, initialized_store, workspace):
    ...
    assert "file_000.txt" in result.stderr
    assert "file_099.txt" in result.stderr
...
def test_ls_tree_long_format_shows_hashes(cli, initialized_store, sample_tree):
    ...
    assert (
        re.search(hex_pattern, result.stderr) is not None
        or "file1.txt" in result.stderr
    )
...
def test_ls_empty_directory_tree(cli, initialized_store, workspace):
    ...
    assert "empty_subdir" in result.stderr or result.returncode == 0
...
def test_ls_unicode_filenames(cli, initialized_store, workspace):
    ...
    assert result.returncode == 0
    assert "café.txt" in result.stderr
    assert "日本語.txt" in result.stderr
